...
HWTEST_F(ZeroBranchTest, Telephony_CellularCallImsControl_002, Function | MediumTest | Level3)
{
    ...
    ASSERT_EQ(imsControl.UpdateImsRttCallMode(SIM1_SLOTID, 0, ImsRTTCallMode::LOCAL_REQUEST_UPGRADE),
        INVALID_VALUE);
#endif
...
#else
#ifdef SUPPORT_RTT_CALL
    ASSERT_NE(imsControl.UpdateImsRttCallMode(SIM1_SLOTID, 0, ImsRTTCallMode::LOCAL_REQUEST_UPGRADE),
        TELEPHONY_SUCCESS);
#endif
...
HWTEST_F(ZeroBranchTest, Telephony_CellularCallConnectionIms_002, Function | MediumTest | Level3)
{
    ...
    int32_t callId = 0;
#ifdef CALL_MANAGER_AUTO_START_OPTIMIZE
    ASSERT_EQ(callConn.UpdateImsRttCallModeRequest(SIM1_SLOTID, callId, ImsRTTCallMode::LOCAL_REQUEST_UPGRADE),
        INVALID_VALUE);
#else
    ASSERT_EQ(callConn.UpdateImsRttCallModeRequest(SIM1_SLOTID, callId, ImsRTTCallMode::LOCAL_REQUEST_UPGRADE),
        TELEPHONY_SUCCESS);
 ...
 * @tc.name     Test UpdateImsVoiceCapabilities with NR mode in valid range
 * @tc.desc     Function test - networkMode is within NR_MODE_MIN and NR_MODE_MAX range,
 *              should add both VoNR and VoLTE capabilities
 */
HWTEST_F(ZeroBranchTest, Telephony_CellularCallConfig_UpdateImsVoiceCapabilities_001, Function | MediumTest | Level3)
{
    ...
    ASSERT_EQ(imsCapabilityList.imsCapabilities.size(), 2);
    bool hasVolte = false;
    bool hasVonr = false;
    for (const auto& cap : imsCapabilityList.imsCapabilities) {
        if (cap.imsCapabilityType == ImsCapabilityType::CAPABILITY_TYPE_VOICE) {
            if (cap.imsRadioTech == ImsRegTech::IMS_REG_TECH_LTE) {
                hasVolte = true;
            } else if (cap.imsRadioTech == ImsRegTech::IMS_REG_TECH_NR) {
                hasVonr = true;
            }
        }
    }
    ASSERT_TRUE(hasVolte);
    ASSERT_TRUE(hasVonr);
}
 
/**
 * @tc.number   Telephony_CellularCallConfig_UpdateImsVoiceCapabilities_002
 * @tc.name     Test UpdateImsVoiceCapabilities with NR mode out of range
 * @tc.desc     Function test - networkMode is outside NR_MODE_MIN and NR_MODE_MAX range,
 *              should only add VoLTE capability
 */
...
/**
 * @tc.number   Telephony_CellularCallConfig_UpdateImsVoiceCapabilities_004
 ...
 */
HWTEST_F(ZeroBranchTest, Telephony_CellularCallConfig_UpdateImsVoiceCapabilities_004, Function | MediumTest | Level3)
{
    CellularCallConfig config;
    ImsCapabilityList imsCapabilityList;
    config.volteSupported_[INVALID_SLOTID] = true;
...
/**
 * @tc.number   Telephony_CellularCallConfig_UpdateImsVoiceCapabilities_005
 * @tc.name     Test UpdateImsVoiceCapabilities with VoNR disabled
 * @tc.desc     Function test - VoNR is disabled, should not add VoNR capability
 */
HWTEST_F(ZeroBranchTest, Telephony_CellularCallConfig_UpdateImsVoiceCapabilities_005, Function | MediumTest | Level3)
{
    CellularCallConfig config;
    ImsCapabilityList imsCapabilityList;
    int32_t VONR_SWITCH_STATUS_OFF = 0;
    ...
    for (const auto& cap : imsCapabilityList.imsCapabilities) {
        if (cap.imsCapabilityType == ImsCapabilityType::CAPABILITY_TYPE_VOICE
            && cap.imsRadioTech == ImsRegTech::IMS_REG_TECH_NR) {
            hasVonr = true;
            break;
        }
    }
...
/**
 * @tc.number   Telephony_CellularCallConfig_UpdateImsVoiceCapabilities_006
 ...
 */
HWTEST_F(ZeroBranchTest, Telephony_CellularCallConfig_UpdateImsVoiceCapabilities_006, Function | MediumTest | Level3)
{
    CellularCallConfig config;
