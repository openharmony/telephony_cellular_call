...
int32_t ImsCallCallbackProxy::GetImsSuppExtResponse(int32_t slotId, int32_t code, int32_t callId)
{
    MessageParcel in;
    int32_t ret = WriteCommonInfo(slotId, __FUNCTION__, in);
    if (ret != TELEPHONY_SUCCESS) {
    ...
    return SendResponseInfo(static_cast<int32_t>(ImsCallCallbackInterfaceCode::IMS_SUPP_EXT_CHANGED), in);
}
